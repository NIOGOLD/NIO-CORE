...
// File contracts/NioToken.sol

pragma solidity ^0.6.0;



contract NioToken is Context, IERC20, Ownable {
    ...
    constructor (address mdexRouter,address _feeManager,address _feeDestruction) public {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256/*tManagement*/) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tManagement, uint256 tDestruction) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tManagement, tDestruction, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tManagement, tDestruction);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tManagement).sub(tDestruction);
        return (tTransferAmount, tFee, tManagement, tDestruction);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tManagement, uint256 tDestruction, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rManagement = tManagement.mul(currentRate);
        uint256 rDestruction = tDestruction.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rManagement).sub(rDestruction);
    ...
    function removeAllFee() private {
        if (_taxFee == 0 && _liquidityFee == 0 && _managementFee == 0 && _destructionFee == 0) return;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tManagement,uint256 tDestruction) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        ...
        emit Transfer(sender, feeMgr, tManagement);
